...
export function useFormInput({
  ...
  const handleValidation = useCallback(() => {
    const isValid = validate(value, validationRules, formData);
    setIsValid(isValid);
 ...
 * @returns {*}
 */
export function validate(value, validation, formData) {
  const fields = [];
  ...
  switch (typeof validation) {
    case 'object':
      Object.keys(validation).forEach(property => {
        let options;
        if( typeof validation[property] === 'function') {
          options = validation[property](formData);
        } else {
